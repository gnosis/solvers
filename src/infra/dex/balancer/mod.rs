...
impl Sor {
    ...
    fn encode_v3_swap(&self, order: &dex::Order, quote: &dto::Quote) -> Result<Vec<dex::Call>, Error> {
        let paths = quote
            .paths
            .iter()
            .map(|p| {
        ...
        Ok(match order.side {
            Side::Buy => self.v3_batch_router.swap_exact_amount_out(paths, &self.permit2, &self.v3_vault),
            Side::Sell => self.v3_batch_router.swap_exact_amount_in(paths, &self.permit2, &self.v3_vault),
        })
    }
