//! This test ensures that the Balancer SOR solver can quote limit-class orders
//! even when the auction carries no token reference prices, which is the shape
//! of quote auctions since drivers quote with limit orders. Without a reference
//! price the solver cannot convert its gas cost into a sell token fee, so it
//! falls back to a zero fee (the orderbook API estimates the fee for quotes
//! itself) and skips the gas simulation, which would revert for the synthetic
//! quote order anyway (its owner is the zero address, and DEXes refuse to
//! transfer the bought tokens there).
